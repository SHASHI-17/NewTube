...
const CONFIG_TIMER_SECONDS = 10; // How long to wait before redirecting to tweet URL (in seconds)
  ...
  try {
    ...
    try {
      const result = await page.evaluate((selArray) => {
        const tweets = document.querySelectorAll('[role="article"], [data-testid="tweet"]');
        if (tweets.length === 0) return false;
...
async function analyzeTweetSize(page) {
  try {
    const analysis = await page.evaluate(() => {
      ...
      const actions = mainTweet.querySelector('[data-testid="like"]')?.parentElement?.parentElement;
...
async function processProfile(
  ...
  try {
    ...
    const page = xTab || (await browser.pages())[0]; // Use the existing first page instead of creating a new one
    ...
    if (!tweetAnalysis) {
      ...
      if (tweetAnalysis.height < 150) {
        ...
        scrollAmount = 80;
      } else if (tweetAnalysis.height < 250 && tweetAnalysis.textLength < 100) {
        // Small text-only tweet
        scrollAmount = 120;
      } else if (tweetAnalysis.height < 350) {
        // Medium-small tweet
        scrollAmount = 180;
      } else if (tweetAnalysis.height < 500 && tweetAnalysis.imageCount === 0 && !tweetAnalysis.hasVideo) {
        // Medium text tweet
        scrollAmount = 250;
    ...
    if (DO_LIKE) {
      ...
      if (alreadyLiked) {
        ...
        for (let attempt = 1; attempt <= 4; attempt++) {
          ...
          if (attempt < 4) {
            const scrollAdjustment = attempt === 1 ? 0 :
                                     attempt === 2 ? 50 :
                                     attempt === 3 ? -30 : 20;
            await page.evaluate((amt) => window.scrollBy(0, amt), scrollAdjustment);
            await sleepWithJitter(600, accountIndex);
          }
        }
